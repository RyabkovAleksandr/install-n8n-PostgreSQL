...
# Скрипт для развертывания n8n на Ubuntu VPS с PostgreSQL
# Поддерживает Ubuntu 20.04, 22.04, 24.04 LTS
# Автор: Viacheslav Lykov (оригинал), с изменениями
...
update_and_clean_system() {
    print_header "Обновление системы и очистка"

    print_info "Обновление пакетов..."
    apt update && apt upgrade -y

    print_info "Очистка ненужных пакетов..."
    apt-get clean
    apt-get autoremove -y

    if command -v docker &> /dev/null; then
        print_info "Очистка Docker образов..."
        docker image prune -f
    fi

    print_success "Система обновлена и очищена"
}

# Настройка SSH порта
setup_ssh_port() {
    print_header "Настройка SSH порта"

    print_info "Изменение SSH порта на 2222..."
    ...
    print_success "SSH порт изменен на 2222"
}
...
setup_ufw() {
    ...
    print_info "Разрешение портов..."
    ufw allow 2222/tcp
    ufw allow 80/tcp
    ufw allow 443/tcp
    ufw allow 5670/tcp  # Для n8n, хотя он локальный, на всякий случай
...
setup_fail2ban() {
    ...
    # Создание конфигурации для jail.local
...
port = 80,443
filter = traefik-auth
logpath = /path/to/traefik/access.log  # Укажите реальный путь к логам Traefik, если нужно настроить
maxretry = 5

[n8n]
enabled = true
port = 5670
filter = n8n-auth  # Кастомный фильтр, если нужно; иначе адаптировать
logpath = /path/to/n8n/logs  # Укажите реальный путь
maxretry = 5
EOF

    # Для Traefik и n8n может потребоваться кастомные фильтры. Здесь базовая настройка.
    # Создаем простой фильтр для traefik-auth (пример)
    mkdir -p /etc/fail2ban/filter.d
    cat > /etc/fail2ban/filter.d/traefik-auth.conf << EOF
[Definition]
failregex = ^.*"POST /api/users/login HTTP/1.1" 401.*$
ignoreregex =
EOF

    # Аналогично для n8n, если есть аутентификация
...
EOF
    ...
    print_info "Проверка статуса SSHD jail..."
...
# Создание конфигурационных файлов (с PostgreSQL и портом 5670)
create_config_files() {
    ...
    # Создание .env файла с PostgreSQL
...
DB_POSTGRESDB_PASSWORD=secure_password_here  # Измените на безопасный пароль!
EOF

    # Создание docker-compose.yml с PostgreSQL и портом 5670
...
EOF
    ...
    cd "$ORIGINAL_DIR" || print_warning "Не удалось вернуться в исходную директорию: $ORIGINAL_DIR"
}
...
main() {
    ...
    print_header "Установка n8n на Ubuntu VPS с PostgreSQL"
    ...
    print_success "n8n успешно развернут на вашем сервере с PostgreSQL и портом 5670"
    ...
    print_warning "Измените пароль PostgreSQL в .env на безопасный!"
}

# Обработка аргументов командной строки (осталось без изменений)
case "${1:-}" in
    --help|-h)
        ...
        ;;
    --version|-v)
        echo "n8n Installation Script v1.1.0 (с PostgreSQL и изменениями)"
